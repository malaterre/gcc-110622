...
struct TypeInfo {
    ...
    template <typename T> T* AllocateAlignedItems(size_t items, AllocPtr alloc_ptr, void* opaque_ptr) {
     ...
     class AlignedFreer {
     public:         AlignedFreer(FreePtr free_ptr, void* opaque_ptr)       : free_(free_ptr), opaque_ptr_(opaque_ptr) {
  }
      template <typename T>   void operator()(T* aligned_pointer) const {
       FreeAlignedBytes(aligned_pointer, free_, opaque_ptr_);
     }
     private:   FreePtr free_;
      void* opaque_ptr_;
    };
     template < typename T > using AlignedFreeUniquePtr = std::unique_ptr< T, AlignedFreer >;
     template <typename T> AlignedFreeUniquePtr<T[]> AllocateAligned(const size_t items , AllocPtr alloc ,                                           FreePtr free , void* opaque ) {
      return AlignedFreeUniquePtr<T[]>(       detail::AllocateAlignedItems<T>(items, alloc, opaque),       AlignedFreer(free, opaque));
    }
     template <typename T> AlignedFreeUniquePtr<T[]> AllocateAligned(const size_t items) {
     ...
     long Lanes(Simd< double >) {
    return Simd< double >::kPrivateLanes;
    }
    ...
    return Set(d, TFromD< D >{
     ...
     VFromD< Simd< long > > Iota0(Simd< long > d) {
    return Zero(d);
    }
     ...
     template < unsigned long NFrom, class DTo > MFromD< DTo > RebindMask(DTo, Mask128< NFrom > m) {
      return m;
    }
     ...
     void StoreU(Vec64< double > v, double *p) {
    vst1_f64(p, v.raw);
    }
     ...
     template < unsigned long N > Mask128< 1 > IsInf(Vec128< double, N > v) {
      ...
      IsInf___trans_tmp_13 = -(MakeSigned< double >{
  1}
    << 1);
     ...
     Vec< Simd< double > > Inf() {
      RebindToUnsigned< Simd< double > > du;
      Inf___trans_tmp_12 = -MakeSigned< TFromD< Simd< double > > >{
  1};
     ...
     template < class V > auto Eq(V a, V b) {
    return a == b;
    }
     auto Lt(Vec128< long, 1 > a, Vec128< long, 1 > b) {
    return a < b;
    }
     void AssertVecEqual(Vec< Simd< double > > expected,                     Vec< Simd< double > > actual) {
      long N(Simd< double >::kPrivateLanes);
      auto expected_lanes = AllocateAligned< double >(1),        actual_lanes = AllocateAligned< double >(1);
      double *__trans_tmp_23 = expected_lanes.get(),          *__trans_tmp_24 = actual_lanes.get();
      Store(expected, __trans_tmp_23);
      Store(actual, __trans_tmp_24);
      auto info = MakeTypeInfo< double >();
      double __trans_tmp_25 = *expected_lanes.get(),          __trans_tmp_26 = *actual_lanes.get();
      AssertArrayEqual(info, &__trans_tmp_25, &__trans_tmp_26, N);
    }
     void AssertMaskEqual(VecArg< Mask< Simd< double > > > a,                      VecArg< Mask< Simd< double > > > b) {
      Vec< Simd< double > > va = VecFromMask(a), vb = VecFromMask(b);
      AssertVecEqual(va, vb);
      Repartition< char, Simd< double > > d8;
      long N8 = Lanes(d8);
      auto bits_a = AllocateAligned< char >(1);
      auto bits_b = AllocateAligned<char>(N8);
