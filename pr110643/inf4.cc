...
void AssertArrayEqual(TypeInfo ti, void *a, void *b,
                      long c) {
  if (memcmp(a, b, ti.sizeof_t) )
    if (ti.sizeof_t && ti.is_float && c ) {
      double da, db;
      memcpy(&da, a, sizeof da);
      memcpy(&db, b, sizeof db);
      bool sba = std::signbit(da);
      bool ina = std::isnan(da);
      volatile bool zero = 0.0;
      {
        sba &&ina &&zero;
        {
          char msg[] =
              "NEON_WITHOUT_AES, f64x1 lane 0 mismatch: expected '-nan', got "
...
template < typename _Tp, typename _Dp > struct unique_ptr< _Tp[], _Dp > {
  ...
  unique_ptr(_Up __p,
             __enable_if_t< integral_constant< false >::value, _Del > __d)
      : _M_t(__p, __d) {}
...
template < typename T >
using AlignedFreeUniquePtr = unique_ptr< T, AlignedFreer >;
void *AllocateAligned_opaque;
template < typename T > AlignedFreeUniquePtr< T[] > AllocateAligned() {
  return AlignedFreeUniquePtr< T[] >(AllocateAlignedItems< T >(),
