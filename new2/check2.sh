...
g++ -std=c++14 -g -m32 -fexcess-precision=standard -O2  -o works $input -Wfatal-errors -Wall -Wextra -Werror 
if ! test "$?" = "0"; then
  exit 1
fi
( ulimit -t 10; ./works )
...
clang++-16 -std=c++14 -fsanitize=memory -o works $input -Wfatal-errors -Wall -Wextra -Werror 
if ! test "$?" = "0"; then
  exit 1
fi
( ulimit -t 10; ./works )
...
g++ -std=c++14 -fexcess-precision=fast -g -m32 -O2 -o fails $input -Wfatal-errors -Wall -Wextra -Werror 
if ! test "$?" = "0"; then
  exit 1
fi
( ulimit -t 10; ./fails )
