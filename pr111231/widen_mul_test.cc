...
void *AllocateAlignedBytes(size_t s, AllocPtr, void *) {
		void *memptr = NULL;
			int ret = posix_memalign(&memptr, 64, s); (void)ret;
				assert(ret == 0);
					return memptr;
}
...
//  size_t bits;
  ...
  return static_cast< T * >(
      AllocateAlignedBytes(bytes, alloc_ptr, AllocateAlignedItems_opaque_ptr));
}
} struct AlignedFreer {
...
AlignedFreeUniquePtr< T[] > AllocateAligned(size_t items, AllocPtr alloc) {
  ...
  return AlignedFreeUniquePtr< T[] >(
      __trans_tmp_1,
      AlignedFreer(AllocateAligned_free, AllocateAligned_opaque));
}
template < typename T >
...
template < typename T > T LowestValue() { return T(); }
...
Vec128< T, N > SaturatedAdd(Vec128< T, N > a, Vec128< T, N > b) {
  using TW = T;
  for (size_t i = 0; i < N; ++i)
    a.raw[i] = HWY_MIN(HWY_MAX(LowestValue< T >(), TW(a.raw[i]) + b.raw[i]),
                       HighestValue< T >());
  return a;
}
namespace detail {
...
template < class D >
VFromD< D > InterleaveUpper(D d, VFromD< D > a, VFromD< D > b) {
  Half< decltype(d) > dh;
  VFromD< D > ret;
...
inline __attribute__((always_inline)) Vec< DI16 >
SatWidenMulPairwiseAdd(DI16 di16, VU8 a, VI8 b) {
...
void AssertArrayEqual2(const TypeInfo &, const void *, const void *, size_t,
...
void AssertVecEqual(D d, Vec< D > expected, Vec< D > actual,
  ...
  volatile auto dbg1 = expected_lanes.get(); (void)dbg1;
  volatile auto dbg2 = actual_lanes.get(); (void)dbg2;
...
struct TestSatWidenMulPairwiseAdd {
  template < typename TN, class DN > void operator()(TN, DN dn) {
    ...
    auto expected = AllocateAligned< TW >(NW); auto in_a = AllocateAligned< TN >(NN); auto in_neg_b = AllocateAligned< TN >(NN); auto in_b = AllocateAligned< TN >(NN);
    int __trans_tmp_2 = LimitsMax< TN >();
    int __trans_tmp_3 = LimitsMax< MakeUnsigned< TN > >();
    int __trans_tmp_4 = LimitsMax< TW >();
    auto vn_signed_min = Set(dn, TN()); auto vn_signed_max = Set(dn, __trans_tmp_2); auto
         vn_unsigned_max = Set(dn_u, __trans_tmp_3); auto
         vw_signed_min = Set(dw, TW()); auto vw_signed_max = Set(dw, __trans_tmp_4);
