...
char AssertVecEqual_target_name;
     ...
     namespace {
     namespace detail {
     constexpr size_t ScaleByPower(size_t N, int pow2) {    return N >> -pow2;    }
     }
    ...
    , detail::ScaleByPower(kWhole, kPow2));
       static constexpr size_t MaxBytes() {    return kPrivateLanes * sizeof(Lane);    }
       template < typename NewT > static constexpr size_t RepartitionLanes() {        return kPrivateLanes / sizeof(NewT);      }
       template < int kNewPow2, size_t kNewMaxLanes >   static constexpr size_t WholeN() {        return detail::ScaleByPower(kNewMaxLanes, kNewPow2);      }
       template < int kNewPow2, size_t kNewMaxLanes >   static constexpr size_t NewN() {        return WholeN< kNewPow2, kNewMaxLanes >();      }
     ...
     namespace detail {
     template < typename T, size_t N, int kPow2 > struct ClampNAndPow2 {      using type = Simd< T, HWY_MIN(N, 36), HWY_MIN(kPow2, 3) >;    };
     template < typename T, size_t kLimit, int kPow2 > struct CappedTagChecker {      static constexpr size_t kLimitPow2 = kLimit;      static constexpr size_t N = HWY_MIN(kLimitPow2, HWY_LANES());      using type = typename ClampNAndPow2< T, N, kPow2 >::type;    };
     }
...
template < class D > size_t MaxLanes(D) {
     ...
     template < int kBits, typename T, size_t N > Vec128< T, N > ShiftLeft(Vec128< T, N > v) {
       for (size_t i = 0;
     i < N;
     ++i) {        T raw_u(v.raw[i]);        auto shifted = raw_u << kBits;        v.raw[i] = T(shifted);      }
       return v;
     }
     template < int kBits, typename T, size_t N > Vec128< T, N > ShiftRight(Vec128< T, N > v) {
       ...
       for (size_t i = 0;
     i < N;
     ++i) {        TU shifted(v.raw[i] >> kBits);        TU upper{};        v.raw[i] = shifted | upper;      }
       return v;
     }
     ...
     namespace detail {
     template < typename T, size_t N > Vec128< T, N > Mul(int, Vec128< T, N > a, Vec128< T, N > b) {      for (size_t i = 0;    i < N;    ++i)     a.raw[i] = a.raw[i] * b.raw[i];      return a;    }
     }
     ...
     template < class D > VFromD< D > InterleaveUpper(D d, VFromD< D > a, VFromD< D > b) {
       ...
       for (size_t i = 0;
     i < MaxLanes(dh);
     ++i) {        ret.raw[2 * i] = a.raw[i];        ret.raw[2 * i + 1] = b.raw[MaxLanes(dh) + i];      }
       return ret;
     }
...
template < typename T, size_t kMul, class Test > struct ForeachCappedR {
     ...
     struct TestSatWidenMulPairwiseAdd {
       template < typename TN, class DN > void operator()(TN, DN dn) {
         ...
         for (size_t i = 0;
     i < NN;
     i++) {          auto val(kSignedMax), a_val(0 ? val : kZeroIotaRepl);          auto b_val(20);          in_a[i] = a_val;          in_b[i] = in_neg_b[i] = TN(-b_val);        }
