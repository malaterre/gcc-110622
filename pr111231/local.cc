...
void FreeAlignedBytes(void *p) { free(p); }
template <typename T>
...
void AssertVecEqual(D d, Vec<D> expected, Vec<D> actual, const char *filename,
                    int line) {
  size_t N = Lanes(d);
  ...
  AssertArrayEqual2(info, expected_lanes.get(), actual_lanes.get(), N,
...
template <int kPow2 = 1> struct ForShrinkableVectors {
  template <typename T> void operator()(T) {
    ...
    ForeachCappedR<T, kMaxCapped, TestSatWidenMulPairwiseAdd>::Do(kMinLanes,
                                                                  max_lanes);
  }
};
