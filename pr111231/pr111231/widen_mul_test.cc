...
struct TestSatWidenMulPairwiseAdd {
  template < typename TN, class DN > void operator()(TN, DN dn) {
    ...
    auto expected = AllocateAligned< TW >(NW), in_a = AllocateAligned< TN >(NN),
         in_neg_b = AllocateAligned< TN >(NN), in_b = AllocateAligned< TN >(NN);
    int __trans_tmp_2 = LimitsMax< TN >();
    int __trans_tmp_3 = LimitsMax< MakeUnsigned< TN > >();
    int __trans_tmp_4 = LimitsMax< TW >();
    auto vn_signed_min = Set(dn, TN()), vn_signed_max = Set(dn, __trans_tmp_2),
         vn_unsigned_max = Set(dn_u, __trans_tmp_3),
         vw_signed_min = Set(dw, TW()), vw_signed_max = Set(dw, __trans_tmp_4);
